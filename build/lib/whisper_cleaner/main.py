...
def main():
    ...
    # If no input directory is given, use the current directory
    if len(args) < 2:
        print("Too few arguments. Using current directory as input directory.")
        input_directory = "." # Use current directory
        model_size = "base" # Use base model
    else:
        print("Using " + args[0] + " as input directory.")
        input_directory = args[0] # Use first command line argument as input directory
        input_directory = input_directory.replace('"', '') # Remove quotes from input directory
        input_directory = input_directory.replace("'", "") # Remove quotes from input directory
        input_directory = str(input_directory)

        model_size = args[1] # Use second command line argument as model size
    ...
    ### GET FILES ###
    # Get all files
    print("Getting mp3 files..." + input_directory)
